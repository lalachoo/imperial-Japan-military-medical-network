...
def split_affiliations(unit_text):
    ...
    mil_parts, gen_parts = [], []
    mil_keywords = r'軍|兵|隊|師団|連隊|防疫|海軍|陸軍|病院|医局|戦|俘虜|731|衛生'
    for part in parts:
        if re.search(mil_keywords, part): mil_parts.append(part)
        else: gen_parts.append(part)
    return ", ".join(gen_parts) if gen_parts else "Unknown", ", ".join(mil_parts) if mil_parts else "Unknown"
GROUP_COLORS = {
    "army": "#C0504D",
    "navy": "#4472C4",
    "mil_med": "#E0A000",
    ...
    for file in xlsx_files:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                col_major = clean_text(row_dict.get("분야", "") or row_dict.get("分野", "-"))
